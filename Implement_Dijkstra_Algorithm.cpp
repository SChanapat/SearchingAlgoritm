  ...
  while ((!candidates.empty()) && (lastSelected.nodeName != w))
  {
    ...
    for (list<char>::iterator i = candidates.begin(); i != candidates.end(); ++i)
    {
      ...
      if (!p.contains(n)) // Add candidate to priority queue if doesn't exist
        p.insert(n);
      else if (p.isBetter(n)) // Update candidate minDist in priority queue if a better path was found
        p.chgPriority(n);
    }
...
void MonteCarlo::run(Graph g)
{
  ...
  cout << endl
       << "=== RUNNING SIMULATION No. " << ++turn << " ===" << endl;
  ...
  if (reachVert != 0)
    ...
    avgPathSize = 0;
  cout << endl
       << "AVG ShortestPath Size (reachVert: " << reachVert << " - sumPathSize: " << sumPathSize << "): " << avgPathSize << endl;
}
